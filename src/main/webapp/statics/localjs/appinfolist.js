...
$(".addVersion").on("click",function(){
	var obj = $(this);
	window.location.href="getappversionadd?id="+obj.attr("appinfoid");
});
...
$(".modifyAppInfo").on("click",function(){
	...
	var status = obj.attr("status");
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href="getappinfomodify?id="+ obj.attr("appinfoid");
	}else{
	...
	$.ajax({
		...
		success:function(data){
			...
			if(data.errorCode === '0'){
				...
				}else if(data.resultMsg === "failed"){//上、下架失败
					if("open" === obj.attr("saleSwitch")){
...
$('#js-export').click(function(){
    ...
    window.location.href= "export?currentPageNo="+currentPageNo;
});

$('#js-exportAll').click(function(){
